...
#            data_folder=data_folder, model_name='park_model2')
#mod.save_settings()
#mod.train_model()

mod = Model.load_model(data_folder + "models/park_model2")
#mod.test_model()
...
camera = camera3
#mod.test_model()
objectrec = ObjectRecognition(mod, ['Car', 'Park'], auto_find=False, visualize=True)
...
while True:
    ...
    objectrec.save_images_from_poi(frame, data_folder + 'new_training_data/', every_x_s=600)
    #print("new image processed in: " + str(round(t-time.time(), 4)) + " seconds")
    cv2.imshow('main', img)
    elapsed_time = time.time() - start_time
    elapsed_time2 = time.time() - start_time2
    if elapsed_time2 >= 600:
        elapsed_time2 = 0
        start_time2 = time.time()
    ...
    if elapsed_time >= 60:
        ...
        start_time = time.time()
        try:
            data = {'Cars': counts['Car'], 'Free': len(objectrec.saved_poi) - counts['Car']}
            r = requests.post('http://192.168.51.140:8080/api/v1/gngqqCwoYPqr5qWmUw8v/telemetry',
                              data=json.dumps(data))
        except Exception:
            print('Could not connect to thingsboard!')
